...
async function run() {
  ...
  try {
    ...
    // Ensure Admin Account kawsarbosuniya52@gmail.com exists
    ...
    console.log("Admin account (kawsarbosuniya52@gmail.com) ready.");
      ...
      const sampleRequests = [
        {
          ...
          requestMessage: 'Emergency blood needed for surgical operation. Please help if you match A+.',
          status: 'pending',
          donorName: null,
          donorEmail: null,
          createdAt: new Date()
        },
        {
          ...
          requestMessage: 'Urgent O- negative blood required for accident victim. Blood bank has no stock.',
          status: 'pending',
          donorName: null,
          donorEmail: null,
          createdAt: new Date()
        },
        {
          ...
          requestMessage: 'B+ blood needed for thalassemia patient routine transfusion.',
          status: 'pending',
          donorName: null,
          donorEmail: null,
          createdAt: new Date()
...
const verifyAdminOrVolunteer = async (req, res, next) => {
  ...
  if (!user || (user.role !== 'admin' && user.role !== 'volunteer')) {
    ...
    return res.status(403).send({ message: 'Forbidden: Admin or Volunteer access only' });
...
app.post('/api/auth/register', async (req, res) => {
  const { email, name, avatar, bloodGroup, district, upazila, password, role } = req.body;
  ...
  try {
    ...
    const validRole = (role && ['donor', 'volunteer', 'admin'].includes(role)) ? role : 'donor';
...
app.get('/api/donors/search', async (req, res) => {
  ...
  try {
    ...
    if (!bloodGroup || !district || !upazila) {
      ...
      return res.status(400).send({ message: 'Blood group, district, and upazila are required' });
    ...
    const donors = await usersCollection.find(query)
...
app.post('/api/donation-requests', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (user.status === 'blocked') {
      ...
      return res.status(403).send({ message: 'Blocked users cannot create blood donation requests' });
    ...
    res.status(201).send({ message: 'Donation request created successfully', requestId: result.insertedId });
...
app.get('/api/donation-requests', async (req, res) => {
  ...
  try {
    ...
    const requests = await donationRequestsCollection.find({ status: 'pending' }).toArray();
...
app.get('/api/donation-requests/my', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (recent === 'true') {
      ...
      const requests = await donationRequestsCollection.find(query)
...
app.get('/api/donation-requests/all', verifyToken, verifyAdminOrVolunteer, async (req, res) => {
...
app.put('/api/donation-requests/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (request.requesterEmail !== email && user.role !== 'admin' && user.role !== 'volunteer') {

      return res.status(403).send({ message: 'Forbidden: You do not have permission to update this request' });
    ...
    await donationRequestsCollection.updateOne({ _id: new ObjectId(id) }, updateDoc);
...
app.delete('/api/donation-requests/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (request.requesterEmail !== email && user.role !== 'admin') {
      ...
      return res.status(403).send({ message: 'Forbidden: You do not have permission to delete this request' });
  ...
  try {
    ...
    if (status === 'inprogress') {
      ...
      if (request.status !== 'pending') {
        ...
        return res.status(400).send({ message: 'Request is no longer pending' });
...
app.post('/api/funding/confirm', verifyToken, async (req, res) => {
  ...
  try {
    ...
    res.status(201).send({ message: 'Funding successfully recorded', fundId: result.insertedId });
...
app.get('/api/stats', verifyToken, verifyAdminOrVolunteer, async (req, res) => {
  ...
  try {
    ...
    const fundingData = await fundsCollection.aggregate([
